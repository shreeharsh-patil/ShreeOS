...
libs=(libX11 libXft libXinerama fontconfig freetype pixman-1 drm GL)
runtime_bins=(
  "usr/bin/Xorg"
  "usr/bin/X"
  "usr/bin/xinit"
...
for lib in "${libs[@]}"; do
  if [ -n "$(find_lib "$SHREEOS_SYSROOT" "lib$lib")" ]; then
    printf "  [OK]      SDK library lib%s\n" "$lib"
  else
    printf "  [MISSING] SDK library lib%s\n" "$lib"
    missing_count=$((missing_count + 1))
  fi

  if [ -n "$(find_lib "$SHREEOS_STAGE_ROOT" "lib$lib")" ]; then
    printf "  [OK]      runtime library lib%s\n" "$lib"
  else
    printf "  [MISSING] runtime library lib%s\n" "$lib"
    missing_count=$((missing_count + 1))
  fi
done
